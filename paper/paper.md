...
Genome and exome sequencing can yield tens of thousands of variants in a single rare disease case, although only a small subset can be examined in detail by a multidisciplinary expert panel. Effective interpretation therefore requires more than filtering a Variant Call Format (VCF) file: population allele frequencies, molecular consequences, inheritance, phenotype, published evidence and functional data must be considered together, while uncertainty and reviewer reasoning remain traceable.

At MedHackathon Asia 2026, we designed a framework for collaborative rare disease variant review across Asia. The work covered three linked areas: variant prioritization and preparation of evidence for ACMG/AMP-style assessment, phenotype extraction and expert validation, and secure expert-review system design. In the proposed workflow, complete VCF and clinical data remain within a local or trusted environment. A structured shortlist of approximately 10–20 candidate variants is prepared, and only the information required for authorized review is transferred. Experts assess a smaller set of high-priority variants, record the evidence applied or left unresolved, and retain individual assessments and disagreements alongside the final review outcome. Reusable variant-level evidence may then be shared across institutions and countries under appropriate governance.

The hackathon also distinguished tasks suitable for automation from those requiring confirmation or clinical and family-level judgment, and identified regional differences in population resources, clinical-data availability, interpretation practice, language and access constraints. Requirements were defined for authentication, case-specific permissions, concurrent editing, consensus tracking and audit logging. The existing ExpertBoard prototype provides a starting point but does not yet support the complete workflow. This report presents the proposed architecture, requirements and implementation roadmap as a design and early-prototype study, and outlines priorities for multicountry pilot review, structured evidence exchange, workflow comparison and governance.

# Introduction

Whole-genome sequencing (WGS) and whole-exome sequencing (WES) are increasingly used in the investigation of rare and undiagnosed diseases. After alignment and variant calling, a single case may yield tens of thousands of single-nucleotide variants and small insertions or deletions. Copy-number variants, structural variants, repeat expansions and mitochondrial variants may add further candidates. The central challenge is therefore to reduce this large technical call set to a small, interpretable group of variants that can be assessed in the context of the patient’s phenotype, family history and the suspected disease mechanism.

Variant interpretation draws on a range of reference databases, annotation tools and clinical knowledge resources, including gnomAD, ClinVar, ClinGen, the Ensembl Variant Effect Predictor (VEP), the Human Phenotype Ontology (HPO) and Online Mendelian Inheritance in Man (OMIM) [@Karczewski:2020; @Landrum:2025; @Rehm:2015; @McLaren:2016; @Kohler:2021; @Amberger:2015]. The ACMG/AMP guidelines provide a widely used framework for combining different types of evidence to classify variants as pathogenic, likely pathogenic, of uncertain significance, likely benign or benign [@Richards:2015]. Later studies have refined individual criteria, introduced quantitative models of evidence strength and provided recommendations for calibrating computational evidence [@Tavtigian:2018; @Pejaver:2022]. These developments have improved the consistency of variant assessment, but they do not remove the need for expert judgment.

Population context remains an important consideration. Allele frequencies and the distribution of disease-associated variants differ among populations, and widely used reference datasets do not represent all Asian populations equally. Genome studies in Singapore have shown the value of ancestrally diverse regional cohorts for the interpretation of clinically relevant variants [@Chan:2022]. Japanese resources such as jMorp and TogoVar provide population-frequency and variant information relevant to Japanese cases [@Tadaka:2024; @Mitsuhashi:2022], while T-REx provides reference exome data from the Thai population [@Shotelersuk:2021]. These resources complement one another, but differ in cohort composition, annotation, access arrangements and clinical context. They therefore cannot be treated as interchangeable sources of evidence.

MedHackathon Asia provides a forum for practical collaboration on genomic and health-data challenges in the region. MedHackathon Asia 2026 was held in Singapore from 27 to 31 July 2026 and included projects addressing variant-analysis harmonization, federated analysis environments, population-aware interpretation, genomic data governance and the discovery of regional resources [@MedHackathonAsia:2026]. The meeting followed the first MedHackathon Asia, held in 2025, and continued its emphasis on collaborative development across countries and disciplines [@MedHackathonCommunity:2026].

Against this background, participants in the 2026 meeting developed the design of a network connecting local variant analysis, phenotype review, multidisciplinary expert assessment and the reuse of variant-level knowledge. The aim was not to establish a centralized authority or a single “Asian” variant classification. Rather, the proposed network would allow panels in different countries or institutions to review the same variant, examine evidence recorded by other panels and retain their own assessments. Agreements and differences could then be compared without requiring all panels to adopt a single conclusion. This report describes the resulting workflow, system architecture and requirements, together with an implementation roadmap for further development and multicountry evaluation.
